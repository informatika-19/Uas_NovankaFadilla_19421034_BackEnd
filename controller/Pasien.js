...
exports.simpanPasien = (data) =>
  new Promise((resolve, reject) => {
    ...
    .then(() => {
        resolve({
            sukses: true,
            pesan: 'Berhasil Input Pendaftaran Pasien'
        })
    }).catch(() => {
        reject({
            sukses: false,
            pesan: 'Gagal Input Pendaftaran Pasien'
        })
    })  
}) 

exports.tampilPasien = () =>
  new Promise((resolve, reject) => {
      pasienModel.find()
       .then(data => {
         resolve({
             sukses: true,
             pesan: 'Berhasil Memuat Data Pasien',
             data: data
         })
       }).catch(() => {
           reject({
               sukses: false,
               pesan: 'Gagal Memuat Data Pasien',
               data: []
           })
       })
  })
